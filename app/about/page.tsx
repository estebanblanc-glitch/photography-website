            ...
            <div className="grid md:grid-cols-2 gap-8">
              <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-lg">
                <p className="text-lg italic mb-4">
                  "{config.testimonials[0]?.content || 'Testimonio'}"
                </p>
                <div className="flex items-center">
                  <img
                    src={config.testimonials[0]?.image || 'https://images.unsplash.com/photo-1494790108755-2616b612b786?w=100&h=100&fit=crop&crop=face'}
                    alt={config.testimonials[0]?.name || 'Cliente'}
                    className="w-12 h-12 rounded-full mr-4 object-cover"
                  />
                  <div>
                    <p className="font-semibold">{config.testimonials[0]?.name || 'Cliente'}</p>
                    <p className="text-gray-600">{config.testimonials[0]?.role || ''}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-lg">
                <p className="text-lg italic mb-4">
                  "{config.testimonials[1]?.content || 'Testimonio'}"
                </p>
                <div className="flex items-center">
                  <img
                    src={config.testimonials[1]?.image || 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=100&h=100&fit=crop&crop=face'}
                    alt={config.testimonials[1]?.name || 'Cliente'}
                    className="w-12 h-12 rounded-full mr-4 object-cover"
                  />
                  <div>
                    <p className="font-semibold">{config.testimonials[1]?.name || 'Cliente'}</p>
                    <p className="text-gray-600">{config.testimonials[1]?.role || ''}</p>
                  </div>
                </div>
              </div>
            </div>
